...
(function () {
  ...
  function fixHomepage() {
    ...
    (function injectMembershipAIBanner(){
      ...
      if (pathname !== '' && pathname !== '/index' && pathname !== '/home') return;
      ...
      '<div class="mu-card" style="background:#1C1C1C;border:1px solid rgba(74,127,165,0.15)">' +
        ...
        '<div style="margin-top:14px;display:flex;flex-direction:column;gap:10px">' +
          '<div style="display:flex;align-items:center;gap:10px;background:rgba(74,127,165,0.06);border-radius:10px;padding:10px 12px;border:1px solid rgba(74,127,165,0.1)">' +
            ...
            '<div><div style="font-size:12px;font-weight:700;color:#D0DFEA">Insight</div><div style="font-size:10px;color:#8C8479;margin-top:1px">47 at-risk members flagged</div></div>' +
          '</div>' +
          '<div style="display:flex;align-items:center;gap:10px;background:rgba(74,127,165,0.06);border-radius:10px;padding:10px 12px;border:1px solid rgba(74,127,165,0.1)">' +
          ...
          '<div style="display:flex;align-items:center;gap:10px;background:rgba(74,127,165,0.06);border-radius:10px;padding:10px 12px;border:1px solid rgba(74,127,165,0.1)">' +
            ...
            '<div><div style="font-size:12px;font-weight:700;color:#D0DFEA">Initiative</div><div style="font-size:10px;color:#8C8479;margin-top:1px">2 campaigns drafted</div></div>' +
          '</div>' +
        '</div>' +
      '</div>';
  ...
  function fixAutomationEngine() {
    ...
    if (!document.querySelector('.ae-faq')) {
      var faqItems = [
        ...
        { q: 'How does PropFuel connect to our AMS?', a: 'PropFuel integrates with 60+ external systems \u2014 including Salesforce, Fonteva, iMIS, Impexium, Novi, MemberSuite, Aptify, Personify, NetForum, Wicket, and more. Two-way sync pulls member data in and writes responses back automatically, where your AMS supports it. Our team handles the setup with you, and a Zapier connector is available for less common systems.' },
  ...
  function fixIntegrationsPage() {
    ...
    var heroSub=document.querySelector('.pf-page-hero-sub');if(heroSub){heroSub.textContent='PropFuel connects to 60+ external systems with two-way sync \u2014 pull member data in, push engagement data back. Our team handles the setup with you.';}
  ...
  function renderMembershipAIPageStaging() {
      ...
      var sectionsHTML =
        ...
        '<section class="section dark mai-stats">' +
          ...
          '<div class="container medium" style="margin-top:48px"><div class="stats-grid">' +
            '<div class="stat"><div class="num">8.68%</div><div class="desc">Average first-year revenue growth (vs 0.98% industry avg)</div></div>' +
        ...
        '<section class="mai-security" style="padding:80px 32px;background:linear-gradient(135deg,#1F3A51 0%,#35607E 55%,#4A7FA5 100%);text-align:center;color:#EDE8DF">' +
          '<div style="max-width:880px;margin:0 auto">' +
            ...
            '<p style="font-size:17px;color:#D0DFEA;line-height:1.6;margin:0 auto 32px;max-width:680px">Your data isn’t training anyone’s model. Your members’ personally identifiable information (PII) never enters an AI call. And a human approves every campaign before it ships.</p>' +
    ...
    document.querySelectorAll('.pf-section').forEach(function(s) {
      ...
      if (t) {
        ...
        if (txt.indexOf('Insight') !== -1 || txt.indexOf('Initiative') !== -1 || txt.indexOf('Recommendation') !== -1) {
          s.style.display = 'none';
        }
      }
    });
  ...
  function fixMembershipAIPage() {
    ...
    // Hide original Webflow feature sections (Insight, Initiative & Recommendation Agents)
    document.querySelectorAll('.pf-section').forEach(function(s) {
  ...
  function fixUseCaseDataIntelligence() {
    buildUseCasePage({
      ...
      stats: [
        ...
        { num: '60+', org: 'Connectors', desc: 'AMS integrations that write PropFuel responses back automatically, where supported.' }
      ],
      ...
      faqs: [
        ...
        { q: 'How does the write-back work?', a: 'When a member responds to a PropFuel check-in, the response writes back to your AMS automatically, where your AMS supports it. The specific field mapping is configured during setup \u2014 you control exactly which fields get updated and in which direction.' },
  ...
  function fixAISecurityPage() {
    ...
    html += '<section class="pf-aisec-hero" style="position:relative;padding:120px 48px 96px;background:linear-gradient(135deg,#1F3A51 0%,#35607E 55%,#4A7FA5 100%);overflow:hidden;text-align:center;color:#EDE8DF">' +
      ...
      '<div style="position:relative;max-width:880px;margin:0 auto">' +
        '<p style="display:inline-flex;align-items:center;gap:8px;padding:8px 18px;border-radius:100px;background:rgba(208,223,234,0.12);border:1px solid rgba(208,223,234,0.30);font-size:13px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#D0DFEA;margin-bottom:28px">' +
          ...
          'AI Data Security' +
        '</p>' +
        '<h1 style="font-size:clamp(40px,5.6vw,64px);font-weight:800;letter-spacing:-0.03em;line-height:1.05;margin-bottom:24px;color:#FFFFFF">Three commitments to protect your members’ data.</h1>' +
        '<p style="font-size:clamp(17px,1.6vw,20px);color:#D0DFEA;line-height:1.55;max-width:680px;margin:0 auto 36px">Zero data retention. No personally identifiable information (PII) in AI calls. A human approves every campaign before it ships. Here’s exactly how PropFuel safeguards your data when Membership AI is at work.</p>' +
        '<div style="display:flex;flex-wrap:wrap;gap:14px;justify-content:center;margin-bottom:36px">' +
          '<a href="https://app.propfuel.com/ai-policy" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:8px;padding:16px 32px;font:700 15px/1 \'DM Sans\',sans-serif;border-radius:100px;text-decoration:none;background:linear-gradient(to right,#F47C2C,#FBC02D) !important;background-image:linear-gradient(to right,#F47C2C,#FBC02D) !important;color:#FFFFFF !important;border:none;box-shadow:0 4px 14px rgba(240,90,40,0.25)">Read the full AI Policy <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></a>' +
    ...
    html += '<section class="pf-aisec-cta" style="padding:96px 48px;background:linear-gradient(135deg,#1F3A51 0%,#35607E 55%,#4A7FA5 100%);background-image:linear-gradient(135deg,#1F3A51 0%,#35607E 55%,#4A7FA5 100%);text-align:center;color:#EDE8DF"><div style="max-width:760px;margin:0 auto">' +
      '<h2 style="font-size:clamp(28px,4vw,42px);font-weight:800;color:#FFFFFF;letter-spacing:-0.02em;line-height:1.15;margin-bottom:20px">Have your security team review the details.</h2>' +
  ...
  function renderEventDemoPage() {
    ...
    // Inject the shared .pf-mmct-* style block (guarded — won't duplicate if /mmct
    // is also rendered, but that's not possible since paths are mutually exclusive).
    ...
    main.innerHTML =
      '<div class="pf-mmct">' +
        '<div class="pf-mmct-main">' +
          '<div class="pf-mmct-card">' +
            '<div class="pf-mmct-pill">Event Follow-Up</div>' +
            '<h1 class="pf-mmct-h1">Great meeting you — let’s <span class="accent">put PropFuel to work.</span></h1>' +
            '<p class="pf-mmct-sub">Drop your email below and grab time on our calendar. We’ll show you exactly what PropFuel can do — personalized to your association.</p>' +
            '<div class="pf-mmct-form-wrap">' +
              '<form id="pf-event-demo-form" novalidate>' +
                '<label for="pf-event-demo-email" class="pf-mmct-label">Work Email</label>' +
                '<input type="email" id="pf-event-demo-email" name="email" required autocomplete="email" placeholder="you@yourassociation.org" class="pf-mmct-input">' +
                '<button type="submit" id="pf-event-demo-submit" class="pf-mmct-cta">Schedule My Demo' +
                  '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-left:6px"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>' +
                '</button>' +
                '<p id="pf-event-demo-msg" class="pf-mmct-msg"></p>' +
              '</form>' +
            '</div>' +
          '</div>' +
        '</div>' +
    ...
    var form = document.getElementById('pf-event-demo-form');
    var emailInput = document.getElementById('pf-event-demo-email');
    var submitBtn = document.getElementById('pf-event-demo-submit');
    var msgEl = document.getElementById('pf-event-demo-msg');
    if (form && emailInput && submitBtn) {
      form.addEventListener('submit', function(e) {
        e.preventDefault();
        var email = (emailInput.value || '').trim();
